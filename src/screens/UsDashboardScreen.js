...
function Avatar({ url, initials, size = 72, accentColor, colors }) {
  return (
    <View style={[styles.avatarRing, { width: size + 6, height: size + 6, borderRadius: (size + 6) / 2, borderColor: accentColor + "55" }]}>
      {url
        ? <Image source={{ uri: url }} style={{ width: size, height: size, borderRadius: size / 2 }} />
        : <View style={[{ width: size, height: size, borderRadius: size / 2, backgroundColor: accentColor, alignItems: "center", justifyContent: "center" }]}>
            <Text style={{ fontSize: size * 0.38, fontFamily: "Catamaran_700Bold", color: "#fff" }}>{initials}</Text>
          </View>}
    </View>
  );
}
